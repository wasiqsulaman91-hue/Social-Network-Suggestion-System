...
4.  **Copy Required DLLs**:
    *   After cloning, the project will not run because it is missing the necessary `.dll` files.
    *   Navigate to your SFML installation's binary folder: `C:\SFML\bin`.
    *   Copy all the `.dll` files from this folder.
    *   Paste them directly into the root of the cloned project folder.
...
*   Run the following command to compile the project:
    ```cmd
    g++ -std=c++17 Main.cpp App_Code/*.cpp -I"C:\SFML\include" -L"C:\SFML\lib" -lsfml-graphics -lsfml-window -lsfml-system -o SNA_Run.exe
    ```
